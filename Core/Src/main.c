...
/* USER CODE BEGIN PV */
volatile uint8_t adc_ready = 0;  // Flag to indicate ADC conversion complete
/* USER CODE END PV */
...
void set_motor(float speedr, float speedl) {
    ...
    if (pwm_l > 0) {
        // Moving Forward
        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_1, (uint8_t)pwm_l);
        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_2, 0);
    }
    else {
        // Moving Reverse OR Stopped (0)
        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_1, 0);
        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_2, (uint8_t)(-pwm_l));
    }
}
/* USER CODE END 0 */
...
uint16_t adcBuffer[5];
  ...
  while (1)
  {
	  ...
	  for (int i = 0; i < 5; i++) {
	    ...
	    if(ir_line[0]>0.7f && ir_line[4]>0.7f && mode==0){
	          	corw++;
	            }
	    else{
	    	corw=0; }
       ...
       if(turncount==0){
		  if(ir_line[1]>0.7 && ir_line[2]>0.7 && ir_line[3]>0.7 && ir_line[4]<0.7){
			  corr++;
		   }
		  else{
			  corr=0;}
		 if(corr==2 && objcount==1) {
			  corr=0;
			  set_motor(0.5f,0.5f);
			  HAL_Delay(200);
			  set_motor(-0.5f,0.5f);
